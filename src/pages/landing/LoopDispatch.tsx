...
import { ChevronDown, Truck, MapPin, CheckCircle, BarChart3, Menu, X } from "lucide-react";
...
export default function LoopDispatch() {
  ...
  return (
    <div style={{ background:BG, color:"#E8EDF3", minHeight:"100vh" }}>
      <style>{`@keyframes dup{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}@keyframes dp{0%,100%{opacity:.35}50%{opacity:1}}.dgrd{background:linear-gradient(135deg,#00BFFF,#00FF88);-webkit-background-clip:text;background-clip:text;color:transparent}.dcard{background:rgba(0,191,255,.04);border:1px solid rgba(0,191,255,.1);border-radius:16px;transition:all .25s}.dcard:hover{background:rgba(0,191,255,.08);border-color:rgba(0,191,255,.28);transform:translateY(-3px);box-shadow:0 12px 40px rgba(0,191,255,.07)}.navlnk{color:rgba(255,255,255,.35);transition:color .18s;text-decoration:none;font-size:13px}.navlnk:hover{color:rgba(255,255,255,.8)}`}</style>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, height:64, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 clamp(20px,5vw,40px)", background:sc?`${BG}F2`:"transparent", backdropFilter:sc?"blur(20px)":"none", borderBottom:sc?`1px solid ${A}12`:"1px solid transparent", transition:"all .3s" }}>
        ...
        <div className="hidden md:flex" style={{ display:"flex", alignItems:"center", gap:6, padding:"4px 14px", borderRadius:999, background:`${A}10`, border:`1px solid ${A}20` }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:A, animation:"dp 2s ease-in-out infinite" }}/><span style={{ fontSize:12, fontWeight:700, color:A }}>Loop Dispatch</span>
        </div>
      ...
      <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"80px 20px 60px", position:"relative", overflow:"hidden", animation:"dup .7s ease forwards" }}>
        ...
        <div style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"6px 16px", borderRadius:999, marginBottom:32, background:`${A}12`, border:`1px solid ${A}30`, color:A }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:A, animation:"dp 2s ease-in-out infinite" }}/><span style={{ fontSize:12, fontWeight:700 }}>Logistics Layer · RALD OS</span>
        </div>
        <div style={{ fontSize:48, marginBottom:12 }}>🚚</div>
        <div style={{ fontSize:"clamp(24px,4vw,36px)", fontWeight:900, marginBottom:16, color:A }}>Loop Dispatch</div>
        ...
        <Link href="/developers" style={{ padding:"13px 26px", borderRadius:999, fontWeight:600, fontSize:14, border:"1px solid rgba(255,255,255,.14)", color:"rgba(255,255,255,.7)", textDecoration:"none" }}>Connect your store</Link>
      ...
      <section ref={s1 as any} id="how" style={{ padding:"clamp(32px,5vw,60px) 20px", background:S, opacity:v1?1:0, transform:v1?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          <p style={{ fontSize:11, fontWeight:700, textAlign:"center", marginBottom:32, letterSpacing:"0.14em", textTransform:"uppercase", color:A }}>How it works</p>
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:4 }}>
            {[{icon:"📦",step:"Order placed",sub:"From your store or app"},{icon:"🤖",step:"Best carrier selected",sub:"GIG, Sendbox, Kwik and more"},{icon:"🗺",step:"Driver picks up",sub:"Real-time GPS tracking starts"},{icon:"✅",step:"Delivered & confirmed",sub:"Photo proof sent to you"}].map((s, i) => (
      ...
      <section ref={s2 as any} id="stories" style={{ padding:"clamp(48px,6vw,80px) 20px", opacity:v2?1:0, transform:v2?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:"clamp(22px,3.5vw,36px)", fontWeight:900, color:"#fff", textAlign:"center", marginBottom:40 }}>Businesses saving time and money</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))", gap:14 }}>
            {STORIES.map((s, i) => (
              <article key={i} className="dcard" style={{ padding:24, cursor:"pointer", borderColor:active===i?`${A}40`:`${A}10`, background:active===i?`${A}07`:"rgba(0,191,255,.02)" }} onClick={() => setActive(i)}>
      ...
      <section ref={s3 as any} style={{ padding:"clamp(48px,6vw,80px) 20px", background:S, opacity:v3?1:0, transform:v3?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:"clamp(22px,3.5vw,36px)", fontWeight:900, color:"#fff", textAlign:"center", marginBottom:40 }}>Delivery made simple</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(340px,1fr))", gap:14 }}>
            {FEATURES.map(f => (
              <div key={f.label} className="dcard" style={{ padding:28, display:"flex", gap:16 }}>
      ...
      <section ref={s4 as any} style={{ padding:"clamp(48px,6vw,80px) 20px", textAlign:"center", borderTop:`1px solid ${A}10`, opacity:v4?1:0, transform:v4?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:"clamp(24px,4vw,44px)", fontWeight:900, color:"#fff", marginBottom:12, lineHeight:1.1 }}>Deliver more.<br/>Spend less. Stress less.</h2>
          <p style={{ color:"rgba(255,255,255,.45)", marginBottom:32 }}>Connects with your store in minutes. Free to try.</p>
