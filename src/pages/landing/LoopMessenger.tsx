...
import { ChevronDown, MessageSquare, Wallet, ShieldCheck, Zap, Menu, X } from "lucide-react";
import { useSEO } from "../../hooks/useSEO";

const A = "#FF7A00", BG = "#0F0700", S = "#130900";

const STORIES = [
  { emoji:"💸", name:"Kemi", city:"Lagos", story:"She texted her friend 'send me ₦5k for the owambe'. Her friend held down the message, tapped Send Money, and the money arrived before she even put her phone down." },
  { emoji:"🛒", name:"Market group", city:"Oyo", story:"A group of market women use a Loop Messenger group to collect daily savings. One admin sends the total at night. Everyone sees who's paid and who hasn't. No fighting over records." },
  { emoji:"🔒", name:"Business deal", city:"Accra", story:"Two traders agreed on a deal over Loop. One moved escrow into the chat — money held until goods arrived. When delivery was confirmed with a photo, funds released automatically." },
];

const FEATURES = [
  { icon:MessageSquare, label:"End-to-end encrypted", desc:"Every message is encrypted. Not even RALD can read your conversations. Your business stays private." },
  { icon:Wallet, label:"Send money inside a chat", desc:"Hold a message, tap Send Money. Done. No switching apps. No typing account numbers. Instant settlement." },
  { icon:Zap, label:"Works on 2G", desc:"Compressed for African networks. Voice notes, file sharing and payments work even on slow connections." },
  { icon:ShieldCheck, label:"Built-in escrow", desc:"Hold payment in a chat until work is done. Both parties agree, then funds release. Trust built-in." },
];
...
export default function LoopMessenger() {
  ...
  useSEO({ title:"Loop Messenger — Chat and Send Money in One App | RALD.cloud", description:"Loop Messenger lets you chat and move money in the same conversation. End-to-end encrypted. Built-in escrow. Works on 2G. Send ₦5k to a friend without ever leaving the chat.", url:"https://rald.cloud/messenger", themeColor:A, product:{ name:"Loop Messenger", applicationCategory:"CommunicationApplication", operatingSystem:"Web, Android, iOS" } });
  ...
  return (
    <div style={{ background:BG, color:"#E8EDF3", minHeight:"100vh" }}>
      <style>{`@keyframes mup{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}@keyframes mp{0%,100%{opacity:.35}50%{opacity:1}}.mgrd{background:linear-gradient(135deg,#FF7A00,#FFD400);-webkit-background-clip:text;background-clip:text;color:transparent}.mcard{background:rgba(255,122,0,.04);border:1px solid rgba(255,122,0,.1);border-radius:16px;transition:all .25s}.mcard:hover{background:rgba(255,122,0,.08);border-color:rgba(255,122,0,.28);transform:translateY(-3px);box-shadow:0 12px 40px rgba(255,122,0,.07)}.navlnk{color:rgba(255,255,255,.35);transition:color .18s;text-decoration:none;font-size:13px}.navlnk:hover{color:rgba(255,255,255,.8)}`}</style>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, height:64, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 clamp(20px,5vw,40px)", background:sc?`${BG}F2`:"transparent", backdropFilter:sc?"blur(20px)":"none", borderBottom:sc?`1px solid ${A}12`:"1px solid transparent", transition:"all .3s" }}>
        ...
        <div className="hidden md:flex" style={{ display:"flex", alignItems:"center", gap:6, padding:"4px 14px", borderRadius:999, background:`${A}10`, border:`1px solid ${A}20` }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:A, animation:"mp 2s ease-in-out infinite" }}/><span style={{ fontSize:12, fontWeight:700, color:A }}>Loop Messenger</span>
        </div>
      ...
      <section style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"80px 20px 60px", position:"relative", overflow:"hidden", animation:"mup .7s ease forwards" }}>
        ...
        <div style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"6px 16px", borderRadius:999, marginBottom:32, background:`${A}12`, border:`1px solid ${A}30`, color:A }}>
          <span style={{ width:6, height:6, borderRadius:"50%", background:A, animation:"mp 2s ease-in-out infinite" }}/><span style={{ fontSize:12, fontWeight:700 }}>Messaging Layer · RALD OS</span>
        </div>
        <img src="/loop-messenger-wordmark.png" alt="Loop Messenger" style={{ height:52, maxWidth:280, objectFit:"contain", marginBottom:28, filter:`drop-shadow(0 0 24px ${A}35)` }}/>
        <h1 style={{ fontSize:"clamp(38px,7vw,80px)", fontWeight:900, lineHeight:1.0, letterSpacing:"-0.03em", maxWidth:700, margin:"0 auto 20px" }}>
          Chat. Send money.<br /><span className="mgrd">Same conversation.</span>
        </h1>
        <p style={{ fontSize:"clamp(16px,2vw,19px)", color:"rgba(255,255,255,.48)", maxWidth:520, margin:"0 auto 40px", lineHeight:1.72 }}>
          Stop switching between WhatsApp and your bank app. Loop Messenger puts messaging and payments in one place — end-to-end encrypted, built for African networks.
        </p>
        <div style={{ display:"flex", flexWrap:"wrap", gap:12, justifyContent:"center" }}>
          <Link href="/payrald" style={{ padding:"13px 26px", borderRadius:999, fontWeight:600, fontSize:14, border:"1px solid rgba(255,255,255,.14)", color:"rgba(255,255,255,.7)", textDecoration:"none" }}>How payments work</Link>
        </div>
      ...
      <section ref={s1 as any} style={{ padding:"clamp(32px,5vw,60px) 20px", background:S, opacity:v1?1:0, transform:v1?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))", gap:12 }}>
          {[{v:"<0.5s",l:"Money arrives"},{v:"E2E",l:"End-to-end encrypted"},{v:"2G",l:"Works on 2G"},{v:"24",l:"African languages"}].map(m => (
            <div key={m.l} className="mcard" style={{ padding:20, textAlign:"center" }}>
      ...
      <section ref={s2 as any} id="stories" style={{ padding:"clamp(48px,6vw,80px) 20px", opacity:v2?1:0, transform:v2?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          <p style={{ fontSize:11, fontWeight:700, textAlign:"center", marginBottom:8, letterSpacing:"0.14em", textTransform:"uppercase", color:A }}>How Africans use it</p>
          <h2 style={{ fontSize:"clamp(22px,3.5vw,36px)", fontWeight:900, color:"#fff", textAlign:"center", marginBottom:40 }}>From owambe transfers to business escrow</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))", gap:14 }}>
            {STORIES.map((s, i) => (
              <article key={i} className="mcard" style={{ padding:24, cursor:"pointer", borderColor:active===i?`${A}40`:`${A}10`, background:active===i?`${A}07`:"rgba(255,122,0,.02)" }} onClick={() => setActive(i)}>
      ...
      <section ref={s3 as any} style={{ padding:"clamp(48px,6vw,80px) 20px", background:S, opacity:v3?1:0, transform:v3?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:900, margin:"0 auto" }}>
          ...
          <h2 style={{ fontSize:"clamp(22px,3.5vw,36px)", fontWeight:900, color:"#fff", textAlign:"center", marginBottom:40 }}>Built for trust. Built for Africa.</h2>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(340px,1fr))", gap:14 }}>
            {FEATURES.map(f => (
              <div key={f.label} className="mcard" style={{ padding:28, display:"flex", gap:16 }}>
      ...
      <section ref={s4 as any} style={{ padding:"clamp(48px,6vw,80px) 20px", textAlign:"center", borderTop:`1px solid ${A}10`, opacity:v4?1:0, transform:v4?"none":"translateY(20px)", transition:"all .6s ease" }}>
        <div style={{ maxWidth:480, margin:"0 auto" }}>
          <img src="/loop-messenger-wordmark.png" alt="Loop Messenger" style={{ height:38, objectFit:"contain", margin:"0 auto 24px" }}/>
          <h2 style={{ fontSize:"clamp(24px,4vw,44px)", fontWeight:900, color:"#fff", marginBottom:12, lineHeight:1.1 }}>Stop switching apps.<br/>Chat and pay in one place.</h2>
          <p style={{ color:"rgba(255,255,255,.45)", marginBottom:32 }}>Free to use. Works on any Android or iPhone. Your contacts join automatically when they have RALD ALIA.</p>
